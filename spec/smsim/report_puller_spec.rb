...
describe Smsim::ReportPuller do
  ...
  let(:sender_number_without_country_code){ '0529992080' }
  ...
  describe '#pull_delivery_notifications_and_sms_replies' do
    ...
    it 'should raise GatewayError when response Status is not one of expected strings' do
      XmlResponseStubs.stub_request_with_pull_notifications_response(self, puller.wsdl_url, :status => "asdf")
      lambda { puller.pull_delivery_notifications_and_sms_replies }.should raise_error(Smsim::GatewayError)
    end
    ...
    it 'should return response with status and batch_size if no messages included' do
      XmlResponseStubs.stub_request_with_pull_notifications_response(self, puller.wsdl_url, :status => "OK", :notifications => [])
    ...
    context 'when report contains delivery notifications' do
      it 'should return response with notifications array' do
        XmlResponseStubs.stub_request_with_pull_notifications_response(self, puller.wsdl_url, :status => "OK")
      ...
      it 'should return response with notifications with typecasted properties ' do
        XmlResponseStubs.stub_request_with_pull_notifications_response(self, puller.wsdl_url, :status => "OK", :notifications => notifications)
        report = puller.pull_delivery_notifications_and_sms_replies
        report.status.should == 1
        ...
        m1.reply_to_phone.should == Smsim::PhoneNumberUtils.ensure_country_code(sender_number_without_country_code)
        m1.message_id.should == 'id1234'

        m2 = report.notifications[1]
        m2.gateway_status.should == -2
        m2.phone.should == '972541234568'
        m2.reply_to_phone.should == Smsim::PhoneNumberUtils.ensure_country_code(sender_number_without_country_code)
        ...
        m3.reply_to_phone.should == Smsim::PhoneNumberUtils.ensure_country_code(sender_number_without_country_code)
    ...
    context 'when report contains sms replies' do
      it 'should return response with replies array' do
        XmlResponseStubs.stub_request_with_pull_notifications_response(self, puller.wsdl_url, :status => "OK", :replies => replies, :notifications => [])
        report = puller.pull_delivery_notifications_and_sms_replies
        report.should be_present
        report.status.should == 1
        report.batch_size.should == 3
