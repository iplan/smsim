  ...
  class SmsRepliesParser
    ...
    def http_push(params)
      ['IncomingXML'].each do |p|
        raise Smsim::GatewayError.new(601, "Missing http parameter #{p}. Parameters were: #{params.inspect}", :params => params) if params[p].blank?
      end
      ...
      begin
        ...
        raise Smsim::GatewayError.new(602, "Failed to parse reply push xml: #{e.message}", :xml => params['IncomingXML'])
      end
    end
    ...
    def parse_reply_values_hash(values)
      ...
      [:phone, :text, :reply_to_phone].each do |key|
        raise Smsim::GatewayError.new(601, "Missing sms reply values key #{key}. Values were: #{values.inspect}", :values => values) if values[key].blank?
      end
      ...
      if values[:received_at].is_a?(String)
        begin
          ...
          raise Smsim::GatewayError.new(603, "NotificationDate could not be converted to date. NotificationDate was: #{values[:received_at]}", :values => values)
        end
      else
    ...
    def generate_reply_message_id(from_phone, reply_to_phone, received_at)
